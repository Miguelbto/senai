CREATE DATABASE IF NOT EXISTS db_eletronicos;
USE db_eletronicos;
...
SELECT AVG(preco_unitario) AS media_preco FROM Produtos;


SELECT SUM(quantidade) AS total_itens_vendidos FROM ItensVenda;



SELECT CONCAT(UPPER(nome), ' (', YEAR(data_cadastro), ')') AS PERFIL_REGISTRO
FROM Clientes;


SELECT categoria, COUNT(id_produto) AS quantidade_produtos FROM Produtos
GROUP BY categoria;

SELECT categoria, COUNT(id_produto) AS quantidade_produtos FROM Produtos
GROUP BY categoria HAVING COUNT(id_produto) > 1;



START TRANSACTION;
INSERT INTO Produtos (id_produto, nome_produto, categoria, preco_unitario, estoque)
VALUES (999, 'Smartphone XV', 'Telefonia', 2500.00, 50);
ROLLBACK;
