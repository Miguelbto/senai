...
if (!fs.existsSync(uploadDir)){ //verifica se existe uma pasta com esse nome, retornando true ou false
    fs.mkdirSync(uploadDir); // O node cria a pasta uploads de forma sincrona, garantindo que a foto seja salva em uma pasta que não existe
}

const storage = multer.diskStorage({
  destination: function (req, file, cb) { //o multer passa a requisição(req), dados do arquivo(file) e uma função callback(cb)
    cb(null, uploadDir + '/'); // o albeck avisa que a decisão foi tomada e o primeiro parâmetro null(indica que não houve erros), o segundo é o caminho final da pasta(uploads/)
  },
  filename: function (req, file, cb) { //para que evite um usuario colocar o mesmo nome da foto de outro e assim sobre escrever e atualizar a imagem errada geramos um numero aleatório
    ...
    cb(null, uniqueSuffix + path.extname(file.originalname)); //o patch.extname extrai só a extenção (.png), originalname é o nome da foto no computador do usuário
  }
});
